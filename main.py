...
async def chat(request: ChatRequest):
    try:
        ...
        return ChatResponse(response="I'm KalaX AI. How can I help?", source="local")

@app.post("/draw", response_model=DrawResponse)
async def draw(request: DrawRequest):
    # Simplified fallback for now – you can keep your original draw logic
    return DrawResponse(success=False, drawing_type="error", explanation="Draw endpoint under maintenance")

# ========== THE FIXED PLOT ENDPOINT ==========
@app.post("/plot")
async def plot_expression(request: Request):
    try:
        ...
        # Prepare evaluation function (safe per point)
        def f(x):
            # Replace ^ with **
            e = expr.replace('^', '**')
            allowed = {
