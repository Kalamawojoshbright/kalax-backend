...
Modules: Learn (graphs/chat), Agriculture (Gemini Vision diagnosis), Vision (attendance logging), Tool (measurement)
...
async def log_attendance(request: dict):
    ...
    category = request.get("category", "student")
    if not name or not date or not time:
        return {"error": "Missing fields"}
    attendance_logs.append({"name": name, "date": date, "time": time, "category": category})
    return {"status": "logged", "student": name, "category": category}
...
async def attendance_summary(request: dict):
    ...
    if not filtered:
        return {"summary": "No attendance records found for this date.", "total": 0}
    present = set(log["name"] for log in filtered)
    categories = {}
    for log in filtered:
        cat = log.get("category", "student")
        categories[cat] = categories.get(cat, 0) + 1
    summary = f"On {filter_date or 'all days'}, {len(present)} individuals present: {', '.join(present)}.\nCategories: {categories}"
    return {"summary": summary, "total": len(present), "categories": categories}
