...
# Simple style that always works
plt.rcParams.update({
    'font.size': 12,
    'axes.linewidth': 1.5,
    'figure.dpi': 150,  # Lower than 300 to avoid timeout on free tier
    'savefig.dpi': 150,
    'figure.figsize': (8, 5)
})
...
# ========== ROBUST PLOTTING FUNCTION (FIXED) ==========
def generate_plot_image(expr: str) -> bytes:
    """Synchronous function that generates a plot and returns PNG bytes."""
    try:
        # Determine domain
        ...
        if any(f in expr_lower for f in ['sin', 'cos', 'tan']):
            ...
            x_min, x_max = -6, 6  # smaller range for speed

        x = np.linspace(x_min, x_max, 500)  # fewer points for speed
        ...
        # Create plot
        fig, ax = plt.subplots()
        ax.plot(x, y, 'b-', linewidth=2, label=f'y = {expr}')

        # Axes
        ax.axhline(y=0, color='black', linewidth=1.2)
        ax.axvline(x=0, color='black', linewidth=1.2)
        ax.grid(True, linestyle='--', alpha=0.6)
        ax.set_title(f'y = {expr}', fontsize=14, fontweight='bold')
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.legend()
        ...
        # Return error as text (not PNG) but we'll handle it
        raise Exception(f"Plot generation failed: {str(e)}")

@app.post("/plot")
async def plot_expression(request: dict):
    """Generate a graph – runs in thread pool to avoid blocking."""
    expr = request.get("expression", "").strip()
    if not expr:
        return Response(status_code=400, content="No expression provided")

    try:
        # Run the blocking plot function in a thread pool
        import concurrent.futures
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as pool:
            image_bytes = await loop.run_in_executor(pool, generate_plot_image, expr)
