"""
KalaX Backend - Textbook-quality graphs + Gemini AI
...
# CORS - allow all origins (your frontend already works)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# ========== TEXTBOOK‑QUALITY GRAPH ENDPOINT ==========
@app.post("/plot")
async def plot_expression(request: dict):
    ...
    if any(func in expr_lower for func in ['sin', 'cos', 'tan']):
        ...
        x_ticks = np.arange(-2*np.pi, 2*np.pi+0.1, np.pi/2)
    ...
    # Add arrows to axes (textbook style)
    # Determine axis limits for arrows
    xlim = ax.get_xlim()
    ylim = ax.get_ylim()
    ax.annotate('', xy=(xlim[1], 0), xytext=(xlim[1]-0.5, 0),
                arrowprops=dict(arrowstyle='->', lw=1.5, color='black'))
    ax.annotate('', xy=(0, ylim[1]), xytext=(0, ylim[1]-0.5),
                arrowprops=dict(arrowstyle='->', lw=1.5, color='black'))

    # Major & minor grid
    ax.grid(True, which='major', linestyle='-', linewidth=0.7, color='gray', alpha=0.6)
    ax.grid(True, which='minor', linestyle=':', linewidth=0.4, color='gray', alpha=0.5)
    ax.minorticks_on()

    # Set custom ticks for trigonometric functions
    if x_ticks is not None and ('sin' in expr_lower or 'cos' in expr_lower):
    ...
    # Labels with LaTeX arrows
    ax.set_xlabel(r'$x \rightarrow$', fontsize=15, labelpad=10)
    ax.set_ylabel(r'$\uparrow y$', fontsize=15, labelpad=10, rotation=0)

    # Title
    ax.set_title(f'Graph of $y = {expr}$', fontsize=16, fontweight='bold', pad=15)
    ...
    for xr in x_roots:
        ...
        ax.annotate(f'({xr:.2f},0)', (xr, 0), xytext=(5, 5), textcoords='offset points',
    ...
    if np.abs(x[idx]) < 0.01:
        ...
        if not np.ma.is_masked(y0):
            ...
            ax.annotate(f'(0,{y0:.2f})', (0, y0), xytext=(5, 5), textcoords='offset points',
                        fontsize=10, color='red', weight='bold')

    # Legend
    ax.legend(loc='upper right', frameon=True, fancybox=True, shadow=True, fontsize=12)

    # Adjust limits for a better fit
    y_min, y_max = np.min(y), np.max(y)
    if not np.ma.is_masked(y_min) and not np.ma.is_masked(y_max):
        y_pad = max(1, (y_max - y_min) * 0.1)
        ax.set_ylim(y_min - y_pad, y_max + y_pad)

    fig.tight_layout()

    # Save to PNG and return
    buf = BytesIO()
