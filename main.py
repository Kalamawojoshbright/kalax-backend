...
# ========== SAFE EVALUATION (NO RECURSION) ==========
def safe_eval(expr: str, x: float) -> float:
    """Evaluate a mathematical expression for a single x value."""
    # Replace ^ with **
    expr = expr.replace('^', '**')
...
def generate_plot_image(expr: str) -> bytes:
    """Generate a plot image safely without recursion."""
    try:
        # Determine domain
        ...
        for x in x_vals:
            try:
                y = safe_eval(expr, x)
                if abs(y) > 100:  # Clip extreme values
                    y = None
                y_vals.append(y)
            except Exception:
                y_vals.append(None)
